...
@section('content')
    ...
    <form method="post" action="{{route('yonetim.urun.kaydet',@$entry->id)}}" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="urun_adet">Adet</label>
                    <input type="text" class="form-control" id="urun_adet" name="urun_adet" placeholder="Adet" value="{{old('urun_adet',$entry->detay->urun_adet)}}">
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
        ...
        <div class="checkbox">
            <label>
                ...
                <input type="checkbox" name="goster_gunun_firsati" value="1" {{old('goster_gunun_firsati',$entry->detay->goster_gunun_firsati ? 'checked' : '')}}> Günün Fırsatları'nda Göster
            </label>
        </div>
        <div class="checkbox">
            <label>
                <input type="hidden" name="goster_one_cikan" value="0">
                <input type="checkbox" name="goster_one_cikan" value="1" {{old('goster_one_cikan',$entry->detay->goster_one_cikan ? 'checked' : '')}}> Öne Çıkanlar'da Göster
            </label>
        </div>
        <div class="checkbox">
            <label>
                <input type="hidden" name="goster_cok_satan" value="0">
                <input type="checkbox" name="goster_cok_satan" value="1" {{old('goster_cok_satan',$entry->detay->goster_cok_satan ? 'checked' : '')}}> Çok Satanlar'da Göster
            </label>
        </div>
        <div class="checkbox">
            <label>
                <input type="hidden" name="goster_indirimli" value="0">
                <input type="checkbox" name="goster_indirimli" value="1" {{old('goster_indirimli',$entry->detay->goster_indirimli ? 'checked' : '')}}> İndirimli Ürünler'de Göster
            </label>
        </div>
        <div class="row">
            <div class="col-md-12">
